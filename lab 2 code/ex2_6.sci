// Exercise 2.6: Addition of two signals
clear; clc; clf;
...
// Calculate the sum y(n) = x1(n) + x2(n)
y = x1 + x2;
...
ax1 = gca(); ax1.data_bounds = [-2.5, -3; 4.5, 7];
xgrid(color('gray'));

for i = 1:length(n)
    if x1(i) >= 0 then y_pos = x1(i) + 0.8; else y_pos = x1(i) - 1.2; end
...
xlabel('Sample n'); ylabel('Amplitude');
ax2 = gca(); ax2.data_bounds = [-2.5, -3; 4.5, 7];
xgrid(color('gray'));

for i = 1:length(n)
    if x2(i) >= 0 then y_pos = x2(i) + 0.8; else y_pos = x2(i) - 1.2; end
...
xlabel('Sample n'); ylabel('Amplitude');
ax3 = gca(); ax3.data_bounds = [-2.5, -3; 4.5, 7];
xgrid(color('gray'));

for i = 1:length(n)
    if y(i) >= 0 then y_pos = y(i) + 0.8; else y_pos = y(i) - 1.2; end
