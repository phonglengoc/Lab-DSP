function [yn, yorigin] = multi(x1n, x1origin, x2n, x2origin)
    ...
    for i = 1:length(n)
        ...
        if (n(i) >= n1_start) & (n(i) <= n1_end) then
            val1 = x1n(n(i) + x1origin);
        end
        if (n(i) >= n2_start) & (n(i) <= n2_end) then
            val2 = x2n(n(i) + x2origin);
        end
        yn(i) = val1 * val2;
    end
    ...
    subplot(3, 1, 3); 
    plot2d3(n, yn, style=6);
    e3 = gce(); e3.children.thickness = 3;
    xtitle("Tín hiệu tích y(n) = x1(n) * x2(n)", "n", "y(n)");
    xgrid();
endfunction

[yn, yorigin] = multi([1, 2, 3, 4], 2, [2, -1, 3], 1);

disp("Dãy tín hiệu tích y(n):");
disp(yn);
